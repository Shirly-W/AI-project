...
if __name__ == '__main__':
    ...
    del_url()
    del_junk_words()
    count = CountVectorizer(token_pattern='[#@_$%a-zA-Z\d]{2,}', lowercase=False, max_features=1000)         # create count vectorizer and change values for some variables
    ...
    clf = tree.DecisionTreeClassifier(min_samples_leaf=int(0.01 * len(train_dict["num_of_train"])), criterion='entropy',
                                      random_state=0)
    ...
    model = clf.fit(X_train_bag_of_words, train_dict["y_train"])                                             #construct decision tree
    test_predict=predict_and_test(model, X_test_bag_of_words)                                                #obtain predict results
    for num in range(len(test_dict["num_of_test"])):                                                         #print test results
