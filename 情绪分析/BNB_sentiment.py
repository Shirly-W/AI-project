...
def del_url():
    ...
    for num_train in range(len(train_dict["x_train"])):
        train_dict["x_train"][num_train]=re.sub(r'(https|http)?:\/\/(\w|\d|\.|\/|\?|\=|\&|\%)*\b',' ',train_dict["x_train"][num_train],flags=re.MULTILINE)
    for num_test in range(len(test_dict["x_test"])):
        test_dict["x_test"][num_test]=re.sub(r'(https|http)?:\/\/(\w|\d|\.|\/|\?|\=|\&|\%)*\b',' ',test_dict["x_test"][num_test],flags=re.MULTILINE)
...
if __name__ == '__main__':
    ...
    model = clf.fit(X_train_bag_of_words, train_dict["y_train"])                          #construct BNB model
    ...
    for num in range(len(test_dict["num_of_test"])):                                      #print testing results
        test_predict[num]=test_predict[num].replace("\n","")
